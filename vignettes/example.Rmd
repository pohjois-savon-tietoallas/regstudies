...
read_classes_tibble(regstudies:::charlson_classes) %>% 
  filter(!is.na(regex)) %>% #View()
  mutate(regex2=str_replace_all(regex,"^\\^","")) %>% #View()
...
mc <- make_classify_table(.data = filtered_d,icdcodes = CODE1,diag_tbl = charlson_classes,return_binary = TRUE)
#View(mc)
