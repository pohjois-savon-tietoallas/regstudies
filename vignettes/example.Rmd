...
## Funktio oletuksena
charlson_classes <- read_classes_csv(file = "../data/charlson_classes.csv")
#View(charlson_classes)
filtered_d %>% 
  classify_codes(codes=CODE1,charlson_classes) -> long # ns. tidy standardi
head(long)
#View(long)

## ok, kaytaja haluaa leveassa muodossa
## esimerkkiin
# NOTE: Ei onnistu helposti
filtered_d  %>% 
...
#View(charlson_classes)
#file.show("../data/charlson_classes.csv")

charlson_classes <- read_classes_csv(file = "../data/charlson_classes.csv") # toistoa, ks. ylempää
make_classify_table(.data = filtered_d,icdcodes = CODE1,diag_tbl = charlson_classes,return_binary = TRUE) %>% View()
wide1 <- filtered_d  %>% 
  classify_codes_wide(icdcodes=CODE1,charlson_classes)
#View(wide1)

filtered_d  %>% 
  classify_codes(codes=CODE1,charlson_classes) %>% 
  tidyr::pivot_wider(names_from="class_charlson",
                     values_from="score_charlson",
                     values_fill=0) -> wide2
head(wide2)
